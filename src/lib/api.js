// src/api.js
export const API_BASE = process.env.REACT_APP_API_URL || "https://service.mwalimubank.co.tz";
...
export const getBoardMembers = async (page = 1, limit = 10) => {
  const res = await fetch(`${API_BASE}/board-of-directors?page=${page}&limit=${limit}`);
...
export const getManagementMembers = async (page = 1, limit = 10) => {
  const res = await fetch(`${API_BASE}/management?page=${page}&limit=${limit}`);
...
export const getHeaderUpdates = async (page = 1, limit = 10) => {
  const res = await fetch(`${API_BASE}/header-update?page=${page}&limit=${limit}`);
...
export const getMenuCategories = async (page = 1, limit = 10) => {
  const token = localStorage.getItem("token");
  
  const res = await fetch(`${API_BASE}/menu-categories/all?page=${page}&limit=${limit}`, {
    headers: {
      Authorization: token ? `Bearer ${token}` : "",
    },
  });
...
export const getMenuItems = async (menuCategory, subcategory, route, page = 1, limit = 10) => {
  const token = localStorage.getItem("token");
  
  let url = `${API_BASE}/menu-items/all?page=${page}&limit=${limit}`;
...
export const getWakalas = async (page = 1, limit = 10) => {
  const token = localStorage.getItem("token");
  
  const res = await fetch(`${API_BASE}/wakala/all?page=${page}&limit=${limit}`, {
    headers: {
      Authorization: token ? `Bearer ${token}` : "",
    },
  });
...
export const getFAQs = async (page = 1, limit = 10) => {
  const token = localStorage.getItem("token");
  
  const res = await fetch(`${API_BASE}/faqs/all?page=${page}&limit=${limit}`, {
    headers: {
      Authorization: token ? `Bearer ${token}` : "",
    },
  });
