...
import { getManagementMember, updateManagementMember } from "../lib/api"
...
export default function ManagementEdit() {
  ...
  useEffect(() => {
    const fetchMember = async () => {
      ...
      try {
        setLoading(true)
        const member = await getManagementMember(id)
        setFormData({
        ...
        console.error("Error fetching management member:", err)
        toast({
          title: "Error",
          description: err.message || "Failed to load management member",
          variant: "destructive",
        })
        navigate("/management")
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateManagementMember(id, data)
      toast({
        title: "Success",
        description: "Management member updated successfully!",
      })
      ...
      toast({
        title: "Error",
        description: err.message || "Failed to update management member",
        variant: "destructive",
      })
    } finally {
      setSubmitting(false)
      ...
      <DashboardLayout>
        <div className="w-full bg-background min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-4">Loading management member...</p>
          </div>
        </div>
      </DashboardLayout>
    )
    ...
    <DashboardLayout>
      <div className="w-full bg-background min-h-screen">
        <div className="max-w-3xl p-6 md:p-8 lg:p-10">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-foreground">Edit Management Member</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="pt-4 flex gap-3">
              <Button type="submit" size="lg" className="w-full md:w-auto px-8" disabled={submitting}>
                {submitting ? "Updating..." : "Update Management Member"}
              </Button>
              <Button
                type="button"
                variant="outline"
