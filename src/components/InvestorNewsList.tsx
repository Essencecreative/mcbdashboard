...
export default function InvestorNewsList() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await getInvestorNews(1, 100) // Fetch all (adjust limit as needed)
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({
        title: "Deleted",
        description: "Investor news removed successfully.",
      })
    } catch (err: any) {
      ...
      <div className="w-full bg-white min-h-screen">
        <div className=" p-6 md:p-8 lg:p-10">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Investor News</h1>
              <p className="text-muted-foreground mt-1">
                Manage all investor communications and announcements.
              </p>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-4 text-lg text-gray-600">No investor news yet.</p>
              <p className="text-sm text-muted-foreground">
                Click "Add New News" to create your first announcement.
              </p>
            </div>
          ) : (
